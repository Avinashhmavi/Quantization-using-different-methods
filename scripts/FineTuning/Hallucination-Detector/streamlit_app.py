...
if api_key:
    ...
    st.sidebar.warning("⚠️ Please enter an OpenRouter API key to use this app")
...
st.write("""
This app demonstrates hallucination detection using the **uqlm** library with OpenRouter models.
...
## Use LangChain's ChatOpenAI for OpenRouter (BaseChatModel compatible)
def get_openrouter_llm(model, api_key, temperature=0.0, max_tokens=2048, timeout=60):
    return ChatOpenAI(
        model=model,
        ...
        timeout=timeout,
        # Optionally, you can add extra_headers via http_client if needed
    )
...
async def run_scorer(scorer_method, prompt, model, temperature):
    ...
    try:
        # Create LLM (BaseChatModel compatible)
        ...
        if scorer_method == "Black-Box Scorer":
            ...
            scorers = [
                ...
                llm  # use same LLM as a judge (BaseChatModel)
            ]
